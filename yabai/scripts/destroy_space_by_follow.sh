...
# then follow focus to wherever those windows landed.
#
# Usage: destroy_space_by_follow.sh <selector>
...
cur_now=$(/opt/homebrew/bin/yabai -m query --spaces --space | /opt/homebrew/bin/jq -r '.index')
if [ "$cur_now" != "$before" ]; then
    /opt/homebrew/bin/yabai -m space --focus "$before"
fi
